@extends('layouts.app', ['title' => 'Boarding House Detail'])
...
@section('content')
    ...
    <div id="Gallery" class="swiper-gallery w-full overflow-x-hidden -mb-[38px]">
        <div class="swiper-wrapper">
            @foreach ($boardingHouse->boardingHouseImages as $boardingHouseImage)
                <div class="swiper-slide !w-fit">
                    <div class="flex shrink-0 w-[320px] h-[430px] overflow-hidden">
                        <img src="{{ Storage::url($boardingHouseImage->image) }}" class="w-full h-full object-cover"
            ...
            <div class="swiper-wrapper">
                <div class="swiper-slide !w-fit">
                    <button
                        ...
                        data-target-tab="#Bonus-Tab">Facilities</button>
                </div>
                <div class="swiper-slide !w-fit">
                    <button
                    ...
                    @foreach ($boardingHouse->facilities as $facility)
                        <div
                            ...
                            <div class="flex w-[120px] h-[90px] shrink-0 rounded-[18px] bg-[#D9D9D9] overflow-hidden">
                                <img src="{{ Storage::url($facility->image) }}" class="w-full h-full object-cover"
                                    alt="thumbnails">
                            </div>
                            <div>
                                <p class="font-semibold">{{ $facility->name }}</p>
                                <p class="text-sm text-ngekos-grey">{!! $facility->description !!}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="mt-[2px] text-sm text-ngekos-grey">
                                        {{ $testimonial->created_at->format('j F Y') }}</p>
                                </div>
                            </div>
                            <p class="leading-[26px]">{!! $testimonial->testimonial !!}</p>
                            <div class="flex">
